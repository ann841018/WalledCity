...
public class Enemy : MonoBehaviour {
	...
	void FixedUpdate () {if (canMove == true && EnemyDead == false && Player.Dead == false) {time = time + Time.deltaTime;//一開始不能自動尋路
		...
		if(Player.BeAttack == false){BeAttackFront.SetActive (false);BeAttackBack.SetActive (false);BeAttackLeft.SetActive (false);BeAttackRight.SetActive (false);}//角色被攻擊的方向
		if (BeAtttract == true) {if (TargetDistance[0] > 10)Go (0);else if (TargetDistance[0] <= 10)Attract ();}//被道具吸引
		...
		float angle = transform.eulerAngles.y;aim = aim - face*angle;if (Physics.Linecast (EnemyRayPoint.position, aim, out EnemyHit)) {//朝角色方向射一條射線
			...
			else if (Vector3.Distance (Player.Mei.position, transform.position) > 10){AttackTime = 2;IsAttack = false;EnemyAnim.SetBool ("Alart", false);Go (0);}}//角色在視線內的話持續更新最後看到角色的位置
			...
			else if (EnemyHit.collider.gameObject.tag == "Enemy") {if (TargetDistance[0] > 10) Go (0);else transform.LookAt (Player.Mei.transform);}}//面向角色	
	...
	void Attack(){if (canMove == true && EnemyDead == false && Player.Dead == false) {//攻擊角色
		EnemyAnim.SetBool ("Run", false);EnemyAnim.SetBool ("Check", false);EnemyAnim.SetBool("Serch",false);EnemyAnim.SetBool ("Alart", true);//緊戒動畫
	...
	void Serch(){if (canMove == true && EnemyDead == false && Player.Dead == false && Player.UseMask == false) {//搜尋
		EnemyNav.enabled = true;AttractTime = AttractTime + Time.deltaTime;//被吸引的時間
		...
		EnemyAnim.SetBool ("Alart", false);EnemyAnim.SetBool ("Check", false);EnemyAnim.SetBool ("Run", false);EnemyAnim.SetBool("Serch",true);enemySoundEffect.run = false;//尋找動畫
			if (AttractTime >= 5) {strangetalk = false;foundtalk = false;attacktalk = false;TargetNumber = (int)(Random.Range (1, 3));EnemyAnim.SetBool("Serch",false);AttractTime = 0;}}//超過一定的時間後 隨機前往其他巡邏點重新巡邏
	}
	void Attract(){if (canMove == true && EnemyDead == false && Player.Dead == false) {//被道具吸引
		...
		if (Physics.Linecast (EnemyRayPoint.position, aim, out EnemyHit)) {
			if (EnemyHit.collider.gameObject.tag == "1") {//敵人跟角色之間沒有障礙物
				AttackTime = AttackTime + Time.deltaTime;if (AttackTime >= 2){EnemyAnim.Play("Attack");//子彈CD
				AttractItem.BillboardHP = AttractItem.BillboardHP - 20;	AttackTime = 0;}} 
	...
	void OnTriggerEnter(Collider Other) {
		...
		if (Other.tag == "1" || Other.tag == "0"|| Other.tag == "5") {
			...
			if (Physics.Linecast (EnemyRayPoint.position, aim, out EnemyHit)) {
				...
				BeAtttract = true;Target[0].position = Other.gameObject.transform.position;}
			}
		}
